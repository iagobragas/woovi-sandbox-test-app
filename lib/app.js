...
async function handleRequest(context) {
  ...
  if (context.method === 'GET' && requestUrl.pathname === '/health') {
    return context.sendJson(200, {
      ...
      persistence: getPersistenceMode(context.config),
    });
  }

  if (context.method === 'GET' && requestUrl.pathname === '/api/webhooks') {
    const event = requestUrl.searchParams.get('event');
    const persistedWebhooks = await listWebhookEvents(context.config, event);
    const webhooks = persistedWebhooks || (
      event
...
async function handleCreateCharge(context) {
  ...
  try {
    ...
    if (!response.ok) {
      const isHtml = typeof text === 'string' && text.trimStart().startsWith('<');
      const isUnavailable = response.status >= 500;

      return context.sendJson(response.status, {
        type: isUnavailable ? 'woovi_unavailable' : 'woovi_error',
        status: response.status,
        error: isUnavailable
          ? `A API Woovi está temporariamente indisponível (${response.status}). Tente novamente em alguns instantes.`
          : 'A Woovi recusou a requisição.',
        details: isHtml ? null : (data?.error || data?.message || data || null),
      });
    }
...
async function handleWooviWebhook(context) {
  try {
    ...
    if (!signatureValid) {
      console.log('Webhook rejected: invalid signature', JSON.stringify({
        sourceIp,
        ipAllowed,
    ...
    if (!ipAllowed) {
      console.log('Webhook rejected: IP not allowed', JSON.stringify({
        sourceIp,
        allowlist: context.config.WOOVI_WEBHOOK_IP_ALLOWLIST,
        event: payload && payload.event ? payload.event : null,
      }));
    ...
    await insertWebhookEvent(context.config, entry);
